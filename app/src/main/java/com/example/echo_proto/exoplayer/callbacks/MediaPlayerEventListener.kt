...
class MediaPlayerEventListener(
    ...
    override fun onPlayerStateChanged(playWhenReady: Boolean, playbackState: Int) {
        ...
        // Обрабатываем завершение воспроизведения эпизода (конец плейлиста)
        if (playbackState == Player.STATE_ENDED) {
            mediaService.onEpisodePlaybackEnded(isAutoTransition = false)
        }
    }

    override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
        super.onMediaItemTransition(mediaItem, reason)
        // Автоматический переход к следующему элементу в плейлисте
        if (reason == Player.MEDIA_ITEM_TRANSITION_REASON_AUTO) {
            mediaService.onEpisodePlaybackEnded(isAutoTransition = true)
        }
    }
